...
O sistema deve permitir ao cliente inserir novos pedidos, escolher um cupom de desconto (10% ou 20%) e exibir o valor total de todos
...
n linhas, cada uma contendo uma string com o nome do pedido e um valor em ponto flutuante separados por espaço. O nome do pedido
não contém espaços em branco; Uma linha contendo o cupom de desconto escolhido (10% ou 20%).
...
def main():
    ...
    cupom = (input())
    if cupom == "10%":
        print(f"Valor total: {(total * .9):.2f}")
    elif cupom == "20%":
        print(f"Valor total: {(total * .8):.2f}")
